...
public class JWTUtils {
	 ...
	 *                              private file
	 */
	public static String generateJWTAssertion(String publicKeyFilename, String privateKeyFilename, String aud,
		...
		if (publicKeyFilename == null || "".equals(publicKeyFilename) || privateKeyFilename == null
				|| "".equals(privateKeyFilename) || aud == null || "".equals(aud) || clientId == null
				|| "".equals(clientId) || userId == null || "".equals(userId)) {
		...
		try {
			...
			token = JWT.create().withIssuer(clientId).withSubject(userId).withAudience(aud).withIssuedAt(new Date(now))
					.withExpiresAt(new Date(now + expiresIn * 1000)).withClaim("scope", scopes).sign(algorithm);
